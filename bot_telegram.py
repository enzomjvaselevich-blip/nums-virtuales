...
# TOKEN DIRECTO (cambia esto por tu token de BotFather)
...
def get_countries():
    try:
        ...
        logger.error(f"Error countries: {e}")
        return None
...
def get_numbers(country_code):
    try:
        ...
        logger.error(f"Error numbers: {e}")
        return None
...
")
        ...
        logger.error(f"Error messages: {e}")
        return None
...
async def show_numbers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"Numeros disponibles ({len(numbers)} total):
...
async def show_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if messages is None:
        await query.edit_message_text("Error al cargar mensajes.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Reintentar", callback_data=f"number:{number_url}")], [InlineKeyboardButton("Volver", callback_data="countries")]]))
        return
    if not messages:
        await query.edit_message_text("No hay mensajes aun. Espera unos minutos.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Actualizar", callback_data=f"number:{number_url}")], [InlineKeyboardButton("Volver", callback_data="countries")]]))
        return
    text = "MENSAJES SMS
...
Luego presiona 'Actualizar' para ver el codigo SMS.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Actualizar SMS", callback_data=f"number:{number_url}")], [InlineKeyboardButton("Volver", callback_data="countries")]]),
        parse_mode="Markdown"
    )
...
async def search_service(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not found_numbers:
        await query.edit_message_text(f"No se encontraron numeros con SMS de {service.capitalize()}.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Volver", callback_data="services")]]))
        return
